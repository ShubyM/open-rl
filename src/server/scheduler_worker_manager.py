...
from server.worker_manager import base_model_of, owner_id, runtime_of, trainer_backend, worker_args, worker_env, worker_module
...
def worker_command(worker: Worker) -> tuple[str, list[str]]:
  """The image and command for one worker pod. A group runs one torchrun
  process per device; the Automodel backend runs out of its own image."""
  ...
  if trainer_backend(worker.meta) == "automodel":
  ...
  return os.getenv("OPEN_RL_WORKER_IMAGE", "ghcr.io/gke-labs/open-rl/server:latest"), ["uv", "run", "--no-sync", "torchrun", *torchrun]
...
def accelerator_spec(worker: Worker) -> dict[str, Any]:
  ...
  (data-parallel replicas each hold a whole model, tensor and context
  parallel ranks split one)."""
  if worker.devices <= 1:
    return {"mode": "SingleGPU", "memory": worker.footprint.accelerator}
  parallelism = worker.meta.trainer_parallelism
  per_device = worker.footprint.accelerator_bytes // (parallelism.tp * parallelism.cp)
