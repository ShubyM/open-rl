...
def trainer_backend(meta: TrainingModelMetadata) -> str:
  """automodel when the client asked for tensor or context parallelism, or
  the deployment says so; otherwise the FSDP trainer."""
  parallelism = meta.trainer_parallelism
  if parallelism.tp > 1 or parallelism.cp > 1 or os.getenv("OPEN_RL_TRAINER_BACKEND", "").lower() == "automodel":
    return "automodel"
  return "fsdp"


def trainer_shape_env(meta: TrainingModelMetadata, is_lora: bool) -> dict[str, str]:
  """What a trainer needs to know about its GPUs. One device is today's
  single-process worker. More than one is a torchrun group over exactly that
  many, never time-slice parked; the Automodel backend gets its layout."""
  parallelism = meta.trainer_parallelism
  env = {"OPEN_RL_TRAINER_PARALLELISM": parallelism.spec}
  if parallelism.devices > 1:
    env["OPEN_RL_FSDP_WORLD_SIZE"] = str(parallelism.devices)
    env["OPEN_RL_TIME_SLICING"] = "off"
  ...
  if trainer_backend(meta) == "automodel":
    ...
    env["OPEN_RL_AUTOMODEL_CP"] = str(parallelism.cp)
