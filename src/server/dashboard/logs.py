...
from server.dashboard import kubernetes as data
...
def archive_path() -> str:
  path = os.getenv("OPEN_RL_LOG_ARCHIVE", "/tmp/open-rl-dashboard/logs.sqlite3")
  return path
    ...
    records.append(
      {
        ...
        "node": pod.get("node"),
        "role": pod.get("role") or (pod.get("labels") or {}).get("openrl.io/role", "unknown"),
        "attempt": attempt,
        "severity": severity,
...
def source_key(item: tuple) -> tuple:
  pod, container, previous = item
  return (pod.get("uid") or pod["name"], container["name"], max(0, container.get("restart_count", 0) - int(previous)), previous)


def collection_sources(pods: list[dict], source_offset: int) -> tuple[list, int]:
  work = {
    source_key((pod, container, previous)): (pod, container, previous)
    for pod in pods
    for container in pod.get("containers", [])
    for previous in ([False, True] if container.get("restart_count", 0) else [False])
  }
  work = list(work.values())
  if work:
    offset = source_offset % len(work)
    work = work[offset:] + work[:offset]
  return work[:MAX_SOURCES], max(0, len(work) - MAX_SOURCES)


def collect(run_id: str, pods: list[dict], source_offset: int = 0, *, reads: dict | None = None) -> dict:
  work, omitted = collection_sources(pods, source_offset)
  sources, records = [], []
  api, error = data.k8s_core_v1()
  ...
  def read_source(item):
    ...
    source = {
      ...
      "previous": previous,
      "shared_runtime": bool(pod.get("shared_runtime")),
      "created_at": pod.get("created_at"),
      "role": pod.get("role", "unknown"),
      "runtime_id": pod.get("runtime_id"),
      "status": "ok",
      "tail_limited": False,
      "collected_at": datetime.now(UTC).isoformat(timespec="microseconds"),
    }
    key = source_key(item)
    cached = reads.get(key) if reads is not None else None
    if cached is None:
      ...
      try:
        ...
        # Kubernetes exception bodies may contain response details; retain only the status.
        error_code = str(getattr(exc, "status", None) or "collection_failed")
      cached = (text, error_code, source["collected_at"])
      if reads is not None:
        reads[key] = cached
    text, error_code, source["collected_at"] = cached
    if error_code is not None:
      source.update(status="unavailable", error_code=error_code)
    else:
      try:
        ...
        source.update(status="unavailable", error_code="collection_failed")
    return source, parsed

  with ThreadPoolExecutor(max_workers=8) as executor:
    for source, parsed in executor.map(read_source, work):
      sources.append(source)
      records.extend(parsed)
  retain(run_id, records, sources)
  return {"sources_omitted": omitted, "sources": sources}


def collect_runs(runs: list[dict], source_offset: int = 0) -> None:
  """Read a shared physical source once per sweep, keeping each run's archive intact."""
  remaining = Counter(source_key(item) for run in runs for item in collection_sources(run["pods"], source_offset)[0])
  reads = {}
  for run in runs:
    if not run["pods"]:
      continue
    collect(run["run_id"], run["pods"], source_offset, reads=reads)
    for item in collection_sources(run["pods"], source_offset)[0]:
      key = source_key(item)
      remaining[key] -= 1
...
async def collector(app) -> None:
  ...
  while True:
    try:
      snapshot = await observations.snapshot(get_store())
      if not snapshot["cluster"]["available"]:
        raise RuntimeError("Kubernetes discovery unavailable")
      await asyncio.to_thread(collect_runs, snapshot["runs"], sweep * MAX_SOURCES)
