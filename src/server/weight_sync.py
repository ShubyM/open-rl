...
class TransferEngine(Protocol):
  ...
  def publish(self, run_id: str, version: int, adapter_name: str, tensors: Sequence[WeightTensor], durable_ref: str | None) -> PublishedState: ...


class LoraTensorSelector:
  """Select only adapter tensors from a PEFT model state dict."""

  lora_markers = ("lora_A", "lora_B", "lora_embedding_A", "lora_embedding_B", "modules_to_save")
  ...
  def is_adapter_tensor(self, name: str, adapter_id: str) -> bool:
    if not any(marker in name for marker in self.lora_markers):
      return False
    return f".{adapter_id}." in name or name.endswith(f".{adapter_id}.weight") or f".{adapter_id}.modules_to_save." in name
...
class FileAdapterTransferEngine:
  ...
  def publish(self, run_id: str, version: int, adapter_name: str, tensors: Sequence[WeightTensor], durable_ref: str | None) -> PublishedState:
    ...
    return PublishedState(run_id, version, adapter_name, "file", self.name, len(tensors), durable_ref, write.payload["manifest_path"])
...
class TorchRLVLLMTransferEngine:
  ...
  def publish(self, run_id: str, version: int, adapter_name: str, tensors: Sequence[WeightTensor], durable_ref: str | None) -> PublishedState:
    ...
    return PublishedState(run_id, version, adapter_name, "vllm", self.name, len(tensors), durable_ref)
...
class VLLMLoraTensorTransferEngine:
  ...
  def publish(self, run_id: str, version: int, adapter_name: str, tensors: Sequence[WeightTensor], durable_ref: str | None) -> PublishedState:
    try:
      ...
      manifest = build_vllm_lora_delta_manifest(run_id, version, tensors, adapter_config, checksum=checksum)
      write = self.store.write_delta(manifest, tensors_for_manifest(tensors, manifest))
      try:
        request = lora_delta_request(run_id, version, adapter_name, durable_ref, adapter_config, manifest, write)
      ...
      return self.fallback.publish(run_id, version, adapter_name, tensors, durable_ref)
...
class VLLMLoraTensorHttpTransferEngine(VLLMLoraTensorTransferEngine):
  def __init__(self, vllm_url: str, **kwargs: Any):
    ...
    super().__init__(vllm_url, store=HttpBodyDeltaStore(tensor_dtype), **kwargs)


class VLLMLoraTensorSharedMemoryTransferEngine(VLLMLoraTensorTransferEngine):
  def __init__(self, vllm_url: str, **kwargs: Any):
    tensor_dtype = kwargs.pop("tensor_dtype", None)
    super().__init__(vllm_url, store=SharedMemoryDeltaStore(tensor_dtype), **kwargs)
...
class WeightSyncBridge:
  ...
  def from_env(cls) -> WeightSyncBridge:
    if os.getenv("SAMPLING_BACKEND", "").lower() == "vllm" or "VLLM_URL" in os.environ:
      transport = os.getenv("OPEN_RL_WEIGHT_SYNC_TRANSPORT", "vllm_lora_tensors_ipc")
      kwargs = {
      ...
      if transport in {"vllm_lora_tensors", "vllm_lora_tensors_http"}:
        return cls(transfer_engine=VLLMLoraTensorHttpTransferEngine(os.getenv("VLLM_URL", "http://127.0.0.1:8001"), **kwargs))
      if transport in {"vllm_lora_tensors_ipc", "vllm_lora_tensors_shm"}:
        return cls(transfer_engine=VLLMLoraTensorSharedMemoryTransferEngine(os.getenv("VLLM_URL", "http://127.0.0.1:8001"), **kwargs))
    return cls()

  def publish_for_inference(self, run_id: str, version: int | None, model: Any, adapter_name: str, durable_ref: str | None) -> PublishedState:
    resolved_version = version if version is not None else self.next_version(run_id)
    tensors = self.selector.select(model, run_id)
    return self.transfer_engine.publish(run_id, resolved_version, adapter_name, tensors, durable_ref)
