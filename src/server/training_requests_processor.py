...
Layers, top to bottom. The API server turns API calls into training.commands
and enqueues them. This loop pops them (rank 0 reads the queue and fans each
batch out to the other torchrun ranks), takes the GPU lease when the batch
needs one, and dispatches each command to the Trainer for that model. Results
go back through the store's futures from rank 0.
"""
...
# Failures the loop treats as one bad request: the command is rejected, the
# client's future carries the error, and the next command runs. Anything else
# on a multi-rank worker may have left a collective half-run, so it is fatal.
REQUEST_ERRORS = (ValueError, NotImplementedError, FileNotFoundError, KeyError)


class FatalWorkerError(Exception):
  """An unexpected failure on a distributed worker: the collective group may be
  desynchronized, so the process finishes its answers and exits rather than
  running the next command against a broken group."""
...
def describe_commands(batch: list[Command]) -> str:
  """`op:request_id` per command, matching the API server's enqueue log line."""
...
class Deployment:
  ...
  model_id: set for a dedicated single-model worker, which drains that model's
...
class TrainingRequestsProcessor:
  def __init__(self, store: RequestStore, worker: TrainingWorker, deployment: Deployment, time_slicer: TimeSlicerClient | None = None):
    # A single-model worker with a model_id drains that model's own queue, which
    # only exists in a shared store. A shared LoRA worker rotates over tenants
    # from the round-robin queue even when it is launched with a model_id.
    self.per_model = worker.single_model and deployment.model_id is not None
    if self.per_model and not os.getenv("REDIS_URL"):
      raise RuntimeError("A dedicated trainer needs REDIS_URL so it can share its queue and futures with the API server")
    self.store = store
    self.worker = worker
  ...
  async def run_once(self) -> None:
    ...
    with tracer.start_as_current_span("training_requests_batch") as span:
      ...
      if work:
        ...
        answered, failure = await self.run_batch(work)
        for command in work:
          fallback = {"type": "RequestFailedResponse", "error_message": f"Trainer worker error: {failure}"}
          await self.publish_result(command.request_id, answered.get(command.request_id, fallback))
        if isinstance(failure, FatalWorkerError):
  ...
  async def run_batch(self, work: list[Command]) -> tuple[dict[str, dict[str, Any]], Exception | None]:
    """Run one batch in FIFO order, taking the lease around each run of GPU
    commands. Grouping by needs_gpu keeps checkpoints and forward passes in the
    order the client sent them rather than hoisting all GPU work ahead of it."""
    answered: dict[str, dict[str, Any]] = {}
    try:
      for on_gpu, group in itertools.groupby(work, key=self.needs_gpu):
        group = list(group)
        if on_gpu:
          async with self.gpu_lease():
            ...
            try:
              for command in group:
                request_id, result = await self.handle(command)
                answered[request_id] = result
            finally:
      ...
      # A failure outside any one command (the lease, wake_up, sleep), or a
      # fatal one inside it. The caller fails whatever went unanswered.
      if not isinstance(exc, FatalWorkerError):
        traceback.print_exc()
      return answered, exc
  ...
  async def handle(self, command: Command) -> tuple[str, dict[str, Any]]:
    """Run one command and return (request_id, result). A bad request becomes a
    RequestFailedResponse so the client's future resolves; an unexpected failure
    on a distributed worker is fatal because the collective group is suspect."""
    ...
    try:
      try:
        ...
        traceback.print_exc()
        if is_distributed():
          raise FatalWorkerError(str(exc)) from exc
        return command.request_id, {"type": "RequestFailedResponse", "error_message": str(exc)}
    finally:
      if token:
        otel_context.detach(token)

  def trainer_for(self, model_id: str) -> Trainer:
    try:
      return self.trainers[model_id]
    except KeyError:
      raise ValueError(f"No model '{model_id}' is loaded on this worker") from None

  def reject_conflicting_create(self, model_id: str, wants_full: bool) -> None:
    if model_id in self.trainers:
      raise ValueError(f"Model '{model_id}' already exists on this worker")
    if wants_full != self.worker.full_parameter:
      trains = "full-parameter" if self.worker.full_parameter else "LoRA"
      raise ValueError(f"This worker trains {trains} models; the create request asked for the other kind")
    if self.worker.single_model and self.trainers:
      raise ValueError(f"This single-model worker already hosts '{next(iter(self.trainers))}' and cannot host '{model_id}' too")

  async def dispatch(self, command: Command) -> dict[str, Any]:
    match command:
      case commands.CreateModel():
        self.reject_conflicting_create(command.model_id, command.fine_tuning_type == "full")
        self.trainers[command.model_id] = await asyncio.to_thread(self.worker.create, command)
        result = {
      ...
      case commands.CreateModelFromState():
        self.reject_conflicting_create(command.model_id, command.fine_tuning_type == "full")
        trainer = await asyncio.to_thread(self.worker.restore, command)
        self.trainers[command.model_id] = trainer
        return {
          "base_model": trainer.base_model_name,
          "model_id": command.model_id,
          "fine_tuning_type": command.fine_tuning_type,
          "type": "model_loaded_from_state",
        }
      case commands.ForwardBackward():
        trainer = self.trainer_for(command.model_id)
        result = await asyncio.to_thread(trainer.forward_backward, command.data, command.loss_fn, command.loss_config, command.forward_only)
      ...
      case commands.LoadWeights():
        trainer = self.trainer_for(command.model_id)
        await asyncio.to_thread(self.worker.reload, trainer, command)
      ...
      case commands.DeleteModel():
        trainer = self.trainers.pop(command.model_id, None)
        if trainer is not None:
          await asyncio.to_thread(self.worker.remove, trainer)
...
async def run_training_requests_processor(
  ...
  if is_distributed() and not worker.distributed:
    raise RuntimeError(
      "WORLD_SIZE>1 but this backend is a single-process HF worker; launching an ordinary model under torchrun does "
      "not make it distributed. Use the Automodel backend, or run this worker on one process."
    )
  if leased is None:
