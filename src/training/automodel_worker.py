...
from training.commands import CreateModel, CreateModelFromState, LoadWeights, SaveWeightsForSampler
from training.distributed import barrier, default_device, group_rank, group_size, is_primary, local_rank, world_size
from training.fft_trainer_worker import trainable_model_parameters
from training.trainer_worker import ENABLE_GRADIENT_CHECKPOINTING, Trainer, TrainingWorker, chunk_target_logprob, tmp_dir
...
class AutomodelTrainer(Trainer):
  """One Automodel model, LoRA or full-parameter, sharded over the worker's
  FSDP2 mesh. It runs the shared training algorithm but specialises the pieces
  the sharded, context-parallel forward needs: the DP group and loss scale, the
  hidden-state logprob projection, DTensor-aware gradient clipping, and the
  distributed checkpointer that gathers shards on save."""
  ...
  def __init__(
    ...
    self.default_grad_clip = GRAD_CLIP_NORM
    if torch.cuda.is_available():
      self.input_device = torch.device("cuda", torch.cuda.current_device())
  ...
  def build_optimizer(self, adam_params: dict[str, Any], **kwargs: Any) -> torch.optim.Optimizer:
    return super().build_optimizer(adam_params, foreach=False)
  ...
  def optim_step(self, adam_params: dict[str, Any]) -> dict[str, Any]:
    ...
    result = super().optim_step(adam_params)
    if self.is_lora:
      self.save_adapter()
    return result
  ...
  def make_training_batches(self, data: list) -> list[list[tuple[int, Any]]]:
  ...
  def compute_target_logprobs(self, input_ids: torch.Tensor, attention_mask: torch.Tensor, target_token_ids: torch.Tensor) -> torch.Tensor:
    ...
    if self.cp_size > 1:
      return self.compute_target_logprobs_cp(input_ids, target_token_ids)
    ...
    if attention_mask is not None and bool(attention_mask.all()):
      attention_mask = None
    self.model(input_ids=input_ids, attention_mask=attention_mask, use_cache=False, logits_to_keep=1, **self.forward_kwargs)
    return self.project_target_logprobs(self.final_hidden_states()[:, :seq_len], target_token_ids)
  ...
  def project_target_logprobs(self, hidden: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
    ...
    softcap = getattr(self.model.config.get_text_config(), "final_logit_softcapping", None)
  ...
  def compute_target_logprobs_cp(self, input_ids: torch.Tensor, target_token_ids: torch.Tensor) -> torch.Tensor:
    ...
    owns_cp = bool(getattr(self.model, "_owns_cp_attention", False))
    if owns_cp:
      sharder = self.model.prepare_model_inputs_for_cp(batch)["cp_sharder"]
    ...
    self.model(input_ids=batch["input_ids"], position_ids=batch["position_ids"], use_cache=False, logits_to_keep=1, **aux, **self.forward_kwargs)
    # Padding slots carry an ignore index; they are cut off after the gather.
    local_logprobs = self.project_target_logprobs(self.final_hidden_states(), batch["labels"].clamp_min(0))
  ...
  def get_checkpointer(self) -> Any:
    """One Automodel Checkpointer for this trainer."""
  ...
  def save_state(self, state_path: str, include_optimizer: bool = False, kind: str = "state") -> dict[str, Any]:
    ...
    metadata = self.checkpoint_metadata(kind=kind, has_optimizer=include_optimizer, lora=self.is_lora)
  ...
  def restore_optimizer(self, state_path: str, metadata: dict[str, Any], map_location: Any = "cpu") -> None:
    """Restore the sharded optimizer state. A checkpoint without one fails the
    request rather than resuming with fresh moments unannounced."""
  ...
  def save_adapter(self, alias: str | None = None) -> SamplerWeights:
    ...
    if not self.is_lora:
      raise RuntimeError("A full-parameter Automodel trainer has no adapter to publish; it saves whole checkpoints.")
...
class AutomodelTrainingWorker(TrainingWorker):
  """One Automodel model per process (one torchrun rank of it). initialize
  selects this rank's GPU and validates the CP*TP layout; the FSDP2 mesh is
  built once the base model names its architecture, and create/restore build
  the sharded trainer on top of it."""

  single_model = True
  distributed = True

  def __init__(self, full_parameter: bool = False, tp_size: int = AUTOMODEL_TP, cp_size: int = AUTOMODEL_CP):
    ...
    self.trainer: AutomodelTrainer | None = None
  ...
  def initialize(self, base_model: str | None = None) -> None:
    ...
    if base_model:
      ...
      self.ensure_mesh(AutoConfig.from_pretrained(base_model))
  ...
  def ensure_mesh(self, config: Any) -> None:
  ...
  def build_model(self, base_model_name: str) -> torch.nn.Module:
    """Build and shard the model. Automodel applies adapters inside
    ...
    if RECOMPUTE_NUM_LAYERS > 0:
      install_group_checkpointing(model, RECOMPUTE_NUM_LAYERS)
    if self.is_lora:
  ...
  def make_trainer(self, model_id: str, model: torch.nn.Module, params: list[torch.nn.Parameter]) -> AutomodelTrainer:
    ...
    text_backbone(model).norm.register_forward_hook(trainer.keep_final_norm_output)
    return trainer

  def capture_params(self, model: torch.nn.Module) -> list[torch.nn.Parameter]:
    if not self.is_lora:
      for param in model.parameters():
        param.requires_grad_(True)
    return trainable_model_parameters(model)

  def create(self, command: CreateModel) -> AutomodelTrainer:
    if self.is_lora:
      self.peft_config = self.build_peft_config(command.lora_config)
    model = self.build_model(command.base_model)
    config = command.lora_config if self.is_lora else command.full_config
    torch.manual_seed(config.seed if config.seed is not None else DEFAULT_SEED)
    params = self.capture_params(model)
    trainer = self.make_trainer(command.model_id, model, params)
    model.train()
    self.trainer = trainer
    return trainer

  def read_state_metadata(self, state_path: str) -> tuple[dict[str, Any], str]:
  ...
  def restore(self, command: CreateModelFromState) -> AutomodelTrainer:
    metadata, base_model = self.read_state_metadata(command.state_path)
    if self.is_lora:
      raise RuntimeError("A LoRA Automodel worker restores adapters into a model built by create_model; issue create_model then load_weights.")
    model = self.build_model(command.state_path)
    self.base_model_name = base_model
    params = self.capture_params(model)
    trainer = self.make_trainer(command.model_id, model, params)
    ...
    if command.restore_optimizer:
      trainer.restore_optimizer(command.state_path, metadata, map_location=self.device)
    self.trainer = trainer
    return trainer

  def reload(self, trainer: AutomodelTrainer, command: LoadWeights) -> dict[str, Any]:
    metadata, base_model = self.read_state_metadata(command.state_path)
    if self.is_lora:
      if trainer.model is None:
        raise RuntimeError("A LoRA Automodel worker restores into adapters built by create_model; create the model first.")
      trainer.get_checkpointer().load_model(trainer.model, model_path=command.state_path)
      params = self.capture_params(trainer.model)
    else:
      model = self.build_model(command.state_path)
      trainer.model = model
