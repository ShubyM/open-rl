...
and launch the worker under torchrun with one process per GPU. There is no
single-process mode: Megatron's parallel state needs a process group before it
can be built, even at TP=1.

OPEN_RL_ENABLE_FFT is not redundant. It is read independently by gateway.py,
worker_launch_processor.py and vllm_sampler.py, where it means "full weights,
not LoRA": launch a dedicated trainer per model, report is_lora=False, and start
the sampler reloading whole checkpoints instead of applying adapters. Setting
only OPEN_RL_TRAINER_BACKEND would give a Megatron trainer feeding a sampler
that still expects adapter files, and every rollout would come from the
untouched base model while training looked healthy. trainer_backend() refuses
that pairing rather than letting it run.

Weights reach the sampler the way the FFT worker's do: save_state writes an HF
checkpoint and the sampler reloads it. Correct, but slow for RL, where it
happens every optimizer step. Pushing tensors straight into a live vLLM engine
over NCCL needs changes on the sampler side too, and is left for later.
...
class MegatronTrainingWorker(BaseTrainerWorker):
  ...
  # the same tokens. All three hooks are therefore scoped to the DP group, whose
  # geometry Megatron already knows. When TP == WORLD_SIZE that group has one
  # member and forward_backward degenerates to "every rank computes everything",
  ...
  def shard_group(self) -> Any:
    _, parallel_state = require_megatron()
    return parallel_state.get_data_parallel_group() if parallel_state.model_parallel_is_initialized() else None
  ...
  def forward_backward(self, data: list[Datum], loss_fn: str, loss_config: dict | None = None, model_id: str | None = None) -> dict[str, Any]:
    ...
    res = super().forward_backward(self.model_chunks[0], data, loss_fn, loss_config)
  ...
  def save_model(self, alias: str | None = None) -> dict[str, Any]:
    ...
    save_path = name if os.path.isabs(name) else os.path.join(tmp_dir, "megatron", name)
    metadata = {
