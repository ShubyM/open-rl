...
UNIT_TESTS ?= tests.test_gateway_paths tests.test_accel_timeslicer tests.test_trainer_optimizer_correctness tests.test_worker_manager tests.test_k8s_worker_manager tests.test_redis_store tests.test_cluster_eval_script tests.test_vllm_sampler
...
# pods are deleted (the gateway reuses running pods per model id).
push-to-cluster: skaffold-check
	skaffold run --default-repo=gcr.io/$(GCP_PROJECT) --file-output=.skaffold-build.json
