#!/usr/bin/env bash
# Bring up (or reuse) the real-GPU kind cluster: host GPUs passed through to
# the kind node via nvidia-container-toolkit device injection, NVIDIA DRA
# driver installed, gpu.nvidia.com DeviceClass registered. Idempotent -- safe
# to run before every smoke. Linux with an NVIDIA driver only.
#
# The cluster name is fixed to openrl-gpu (kind reads it from the config).
set -euo pipefail
...
echo "Ensuring kind cluster openrl-gpu exists..."
if ! kind get clusters | grep -Fxq "openrl-gpu"; then
  kind create cluster --config "$script_dir/kind-config.yaml"
fi

echo "Verifying GPU visibility inside the kind node..."
if ! docker exec openrl-gpu-control-plane nvidia-smi -L; then
  echo "GPU is not visible in the kind node. Recheck the runtime config, restart docker, and recreate the cluster."
  exit 1
fi
...
until kubectl --context kind-openrl-gpu get deviceclass gpu.nvidia.com >/dev/null 2>&1; do
...
echo "kind GPU cluster is ready: context kind-openrl-gpu."
