#!/usr/bin/env bash
# Bring the stack up for run42: Qwen3.5-9B on the Megatron backend at TP=2/DP=2,
# 163,840-token context -- run19's recipe on a backend that has never run it.
#
# Separate from run42.sh because these are separate failure modes: this file
# decides the topology the servers come up with, and run42.sh refuses to start
...
echo "[launch42] $MODEL @ $CONTEXT, backend=$TRAINER_BACKEND TP=$MEGATRON_TP (DP=$((TRAIN_GPUS / MEGATRON_TP))) SP=$MEGATRON_SEQUENCE_PARALLEL"
echo "[launch42] LoRA rank=$MEGATRON_LORA_RANK targets=$MEGATRON_LORA_TARGETS"
