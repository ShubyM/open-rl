...
reference. Launch under torchrun with the layout in the usual env vars:
  ...
  PROBE_MODE=ckpt    save_state -> load_from_state round trip reproduces the
                     logprobs; save_adapter emits a PEFT dir; lists its keys.
  ...
  PROBE_MODEL (default Qwen/Qwen3.5-9B), PROBE_LEN (default 4096), PROBE_LORA_RANK (default 32),
...
LORA_RANK = int(os.getenv("PROBE_LORA_RANK", "32"))
...
def build_worker():
  ...
  worker.create_model(MODEL, "probe", LoraConfig(rank=LORA_RANK, lora_alpha=16, lora_dropout=0.0))
...
def mode_ckpt(worker):
  ...
  worker.save_adapter("probe-model", alias="probe")
  adapter_dir = os.path.join(os.getenv("OPEN_RL_TMP_DIR", "/tmp/open-rl"), "peft", "probe-model", "probe-model")
  if dist.get_rank() == 0:
...
def main() -> None:
  ...
  layout = f"TP={os.getenv('OPEN_RL_AUTOMODEL_TP')} CP={os.getenv('OPEN_RL_AUTOMODEL_CP')} rank={LORA_RANK}"
