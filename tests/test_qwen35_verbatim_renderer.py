...
RECIPE_DIR = Path(__file__).resolve().parents[1] / "examples" / "harvey_labs"
...
try:
  ...
  from qwen35_renderer import SAMPLED_TOKENS_KEY, VerbatimHistoryQwen35Renderer, verbatim_history_renderer
...
SAMPLED = {
  "tool_call": "Let me look around.\n</think>\n\n<tool_call>\n<function=bash>\n<parameter=command>\nls -la\n</parameter>\n</function>\n</tool_call><|im_end|>",
  "tool_call_one_line_params": "Check.\n</think>\n\n<tool_call>\n<function=bash>\n<parameter=command>ls -la</parameter>\n</function>\n</tool_call><|im_end|>",
  "two_tool_calls": "Two things.\n</think>\n\n<tool_call>\n<function=bash>\n<parameter=command>\nls\n</parameter>\n</function>\n</tool_call>\n<tool_call>\n<function=bash>\n<parameter=command>\npwd\n</parameter>\n</function>\n</tool_call><|im_end|>",
  "whitespace_in_thinking": "  Let me think...  \n\n</think>\n\n<tool_call>\n<function=bash>\n<parameter=command>\nls\n</parameter>\n</function>\n</tool_call><|im_end|>",
...
class VerbatimHistoryTest(unittest.TestCase):
  ...
  def test_every_sampled_form_extends_the_previous_observation(self) -> None:
    ...
    for name, text in SAMPLED.items():
      with self.subTest(sampled=name):
        ...
        self.assertIsNone(
          i,
          f"chain breaks at token {i}: expected {self.tokenizer.decode(expected[i:i + 8])!r}, "
          f"got {self.tokenizer.decode(nxt[i:i + 8])!r}" if i is not None else None,
        )
        self.assertGreater(len(nxt), len(expected), "the tool result must follow")

  def test_the_stock_renderer_breaks_the_chain_on_tool_calls(self) -> None:
    ...
    sampled = self.sampled_tokens(SAMPLED["tool_call"])
  ...
  def test_a_three_turn_episode_becomes_one_datum(self) -> None:
    ...
    for text in (SAMPLED["tool_call"], SAMPLED["whitespace_in_thinking"], SAMPLED["text_answer"]):
      sampled = self.sampled_tokens(text)
      message, termination = renderer.parse_response(sampled)
