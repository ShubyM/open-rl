...
class TableWorker(BaseTrainerWorker):
  """A worker whose data-parallel group is handed in, the way the Automodel
  worker derives its own from the device mesh. backward runs FSDP collectives,
  so the base loop pads short ranks and scales each real pass by the shard count."""
