...
from training.automodel_worker import AutomodelTrainer, GatherSequenceShards, round_robin_permutation
from training.distributed import group_size
from training.trainer_worker import Datum, Trainer

HIDDEN, VOCAB = 8, 11


def build_trainer(model, params=None, is_lora=False):
  return AutomodelTrainer(
    "math",
    model,
    params if params is not None else list(model.parameters()),
    None,
    "base",
    device_mesh=None,
    cp_size=1,
...
class ProjectTargetLogprobsTest(unittest.TestCase):
  ...
  def check(self, softcap: float | None) -> None:
    ...
    with patch.object(automodel_worker, "LOGPROB_CHUNK", 4):
      actual = trainer.project_target_logprobs(hidden, targets)
...
class ClipGradientsTest(unittest.TestCase):
  ...
  def make_trainer(self):
    ...
    trainer = build_trainer(torch.nn.Linear(1, 1), params=params)
  ...
  def test_clips_like_torch(self) -> None:
    ...
    total, clip_coef = trainer.clip_gradients(0.5)
  ...
  def test_no_clipping_under_the_threshold(self) -> None:
    ...
    trainer.clip_gradients(float("inf"))
...
class RatioStatsTest(unittest.TestCase):
  def test_tail_counts_only_positions_with_an_advantage(self) -> None:
    ...
    trainer = Trainer("ratio", model, list(model.parameters()))
    ...
    trainer.record_ratio_stats(target, old, weights, advantages)
    metrics = trainer.ratio_metrics()
    ...
    self.assertEqual(trainer.ratio_metrics()["ratio/max_abs_log:max"], 0.0)
...
class TableTrainer(Trainer):
  def __init__(self, model, group):
    super().__init__("table", model, list(model.parameters()))
  ...
  def compute_target_logprobs(self, input_ids, attention_mask, target_token_ids):
    return self.model(target_token_ids)
...
def run_fsdp_data_parallel(rank: int, world_size: int, port: int) -> None:
  ...
  try:
    ...
    expected = TableTrainer(reference_model, None).forward_backward(table_data(), "cross_entropy")

    mesh = init_device_mesh("cpu", (world_size,))
    model = fully_shard(TableModel(), mesh=mesh)
    actual = TableTrainer(model, mesh.get_group()).forward_backward(table_data(), "cross_entropy")
