...
A real tiny Llama on CPU, the real in-memory store, the real API server
endpoints, the real request processor and the real LoRA and FFT workers.
Commands enter through the store the way the API server enqueues them and
...
from training.lora_trainer_worker import LoraTrainer, LoraTrainingWorker
...
class LoraHostTest(TinyLlamaCase):
  async def test_two_adapters_train_independently_on_one_base(self) -> None:
    ...
    self.assertIsInstance(processor.trainers["a"], LoraTrainer)
    self.assertIsInstance(processor.trainers["b"], LoraTrainer)
...
class ApiServerToWorkerTest(TinyLlamaCase):
  async def test_api_calls_become_commands_the_worker_answers(self) -> None:
    store = InMemoryStore()
    with patch.object(api_server, "store", store):
      client = await self.enterAsyncContext(asgi_client())
      created = await post_json(client, "create_model", {"base_model": self.base_model, "lora_config": {"rank": 2, "seed": 1}})
      ...
      await processor.run_once()
      self.assertEqual(api_server.translate_future_result(await store.get_future(model_id, timeout=1))["lora_rank"], 2)
      ...
      fb = await post_json(
        client, "forward_backward", {"model_id": model_id, "forward_backward_input": {"data": wire_data, "loss_fn": "cross_entropy"}}
      )
      step = await post_json(client, "optim_step", {"model_id": model_id, "adam_params": {"learning_rate": 1e-2}})
      saved = await post_json(client, "save_weights_for_sampler", {"model_id": model_id, "sampling_session_seq_id": 0})
      await processor.run_once()
