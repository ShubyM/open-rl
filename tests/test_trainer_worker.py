...
class TestFFTWorker(TmpDirCase):
  ...
  def test_save_state_and_load_from_state_round_trip_restores_the_weights(self):
    # Only the weights round-trip today: FFT save_state ignores include_optimizer, so
    # restore_optimizer has nothing to read and a resumed run restarts Adam from zero.
...
def direct_logprobs(model: torch.nn.Module, d: Datum) -> torch.Tensor:
  """The per-token target logprobs computed with nothing but a forward pass and log_softmax."""
  targets = torch.tensor(d.loss_fn_inputs["target_tokens"].data)
  with torch.no_grad():
    logits = model(torch.tensor([d.model_input])).logits[0, : len(targets)].float()
  return torch.log_softmax(logits, dim=-1)[torch.arange(len(targets)), targets]


def reported_logprobs(result: dict) -> list[torch.Tensor]:
  return [torch.tensor(out["logprobs"]["data"]) for out in result["loss_fn_outputs"]]


class TestTrainingCorrectness(TmpDirCase):
  """Numbers the workers report, checked against independent computations.

  These are the checks to run when a worker, peft, or transformers changes: they
  say whether the reported logprobs, losses and update direction are right, not
  merely whether the code ran.
  """

  def lora_worker_with_adapter(self) -> LoraTrainingWorker:
    worker = lora_worker()
    worker.create_adapter("adapter-a", LoraConfig(rank=2, seed=1, lora_dropout=0.0))
    return worker

  def test_fft_reported_logprobs_match_a_direct_forward(self):
    worker = fft_worker()
    result = worker.forward_backward(BATCH, "cross_entropy")

    for d, reported in zip(BATCH, reported_logprobs(result), strict=True):
      torch.testing.assert_close(reported, direct_logprobs(worker.model, d), rtol=1e-5, atol=1e-5)
    # cross entropy with unit weights is minus the sum of those logprobs
    self.assertAlmostEqual(result["metrics"]["loss:sum"], -sum(lp.sum().item() for lp in reported_logprobs(result)), places=4)

  def test_lora_reported_logprobs_match_a_direct_forward(self):
    worker = self.lora_worker_with_adapter()
    result = worker.forward_backward(BATCH, "cross_entropy", None, "adapter-a")

    for d, reported in zip(BATCH, reported_logprobs(result), strict=True):
      torch.testing.assert_close(reported, direct_logprobs(worker.peft_model, d), rtol=1e-5, atol=1e-5)

  def test_a_fresh_adapter_leaves_the_base_model_untouched(self):
    # peft initialises B to zero, so an untrained adapter must reproduce the base model exactly.
    lora = self.lora_worker_with_adapter()
    fft = fft_worker()

    lora_result = lora.forward_backward(BATCH, "cross_entropy", None, "adapter-a")
    fft_result = fft.forward_backward(BATCH, "cross_entropy")

    for from_lora, from_fft in zip(reported_logprobs(lora_result), reported_logprobs(fft_result), strict=True):
      torch.testing.assert_close(from_lora, from_fft, rtol=1e-5, atol=1e-5)

  def test_one_optimizer_step_lowers_the_loss_on_the_same_batch(self):
    lora = self.lora_worker_with_adapter()
    fft = fft_worker()
    for name, worker, args in (("lora", lora, (None, "adapter-a")), ("fft", fft, ())):
      with self.subTest(worker=name):
        before = worker.forward_backward(BATCH, "cross_entropy", *args)["metrics"]["loss:mean"]
        worker.optim_step(ADAM, *args[1:])
        after = worker.forward_backward(BATCH, "cross_entropy", *args)["metrics"]["loss:mean"]
        self.assertLess(after, before)

  def test_loss_functions_match_their_formulas(self):
    lp = torch.tensor([-1.0, -2.0])
    w = torch.tensor([1.0, 0.5])
    old = torch.tensor([-1.5, -1.0])
    adv = torch.tensor([2.0, -1.0])
    ratio = [math.exp(-1.0 + 1.5), math.exp(-2.0 + 1.0)]

    torch.testing.assert_close(losses.cross_entropy_loss(lp, w), torch.tensor([1.0, 1.0]))
    torch.testing.assert_close(
      losses.importance_sampling_loss(lp, w, old, adv),
      torch.tensor([-(ratio[0] * 2.0) * 1.0, -(ratio[1] * -1.0) * 0.5]),
    )
    # clip 0.2: the first ratio (1.65, positive advantage) is clipped to 1.2; the second (0.37,
    # negative advantage) is clipped to 0.8, and min picks the clipped, more pessimistic term.
    torch.testing.assert_close(
      losses.ppo_loss(lp, w, old, adv, {"clip_range": 0.2}),
      torch.tensor([-(1.2 * 2.0) * 1.0, -(0.8 * -1.0) * 0.5]),
    )
    # the kl penalty subtracts kl_coeff * ((ratio - 1) - log ratio) from the objective
    kl = [(r - 1.0) - math.log(r) for r in ratio]
    torch.testing.assert_close(
      losses.ppo_loss(lp, w, old, adv, {"clip_range": 0.2, "kl_coeff": 0.1}),
      torch.tensor([-((1.2 * 2.0) - 0.1 * kl[0]) * 1.0, -((0.8 * -1.0) - 0.1 * kl[1]) * 0.5]),
    )
