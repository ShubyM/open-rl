...
class ParallelismTest(unittest.TestCase):
  ...
  def test_trainer_shape_env_is_empty_for_one_device_and_full_for_a_group(self) -> None:
    from server.model_metadata import Parallelism, TrainingModelMetadata
    ...
    lora = TrainingModelMetadata(base_model="m", fine_tuning_type="lora", trainer_parallelism=Parallelism(tp=2, cp=2))
    env = trainer_shape_env(lora, is_lora=True)
    self.assertEqual(env["OPEN_RL_TRAINER_BACKEND"], "automodel")
    self.assertEqual((env["OPEN_RL_AUTOMODEL_TP"], env["OPEN_RL_AUTOMODEL_CP"], env["OPEN_RL_FSDP_WORLD_SIZE"]), ("2", "2", "4"))
