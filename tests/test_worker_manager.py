...
class LocalWorkerManagerTest(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_local_launch_uses_torchrun_for_fsdp(self) -> None:
    with (
      patch.dict("os.environ", {"REDIS_URL": "redis://localhost:6379", "OPEN_RL_FSDP_WORLD_SIZE": "2"}, clear=True),
      patch("server.worker_manager.subprocess.Popen") as popen,
      patch("server.worker_manager.shutil.which", return_value="/usr/bin/uv"),
    ):
      LocalWorkerManager().launch("model-a")

    command = popen.call_args.args[0]
    self.assertIn("torchrun", command)
    self.assertIn("--nproc-per-node=2", command)
    self.assertIn("--model-id", command)
    self.assertEqual(command[command.index("--model-id") + 1], "model-a")
