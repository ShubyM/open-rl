...
class SchedulerWorkerManagerTest(unittest.TestCase):
  ...
  def test_lora_trainer_and_sampler_share_an_owner(self) -> None:
    ...
    with patch("server.store.get_store", return_value=s):
      self.manager.launch_trainer("job-lora-1")
      self.manager.launch_sampler("job-lora-1")
    ...
    self.assertEqual(trainer["spec"]["accelerator"], {"memory": "10Gi", "maxDeviceCount": 1})
  ...
  def test_fft_worker_is_its_own_owner(self) -> None:
    ...
    self.assertEqual(worker["spec"]["accelerator"]["memory"], "60Gi")
    container = worker["spec"]["template"]["spec"]["containers"][0]
    env = {e["name"]: e.get("value") for e in container["env"]}
    self.assertEqual(env["OPEN_RL_ENABLE_FFT"], "true")
    self.assertEqual(env["OPEN_RL_FINE_TUNING_TYPE"], "full")
    # Identity env agrees with the spec by construction.
    self.assertEqual(env["OPEN_RL_TIME_SLICE_OWNER"], worker["spec"]["ownerId"])
  ...
  def test_shutdown_deletes_both_roles_and_tolerates_absence(self) -> None:
    s = self._store_with("job-lora-1", {"base_model": "Qwen/Qwen2.5-0.5B", "fine_tuning_type": "lora"})
    with patch("server.store.get_store", return_value=s):
      self.manager.launch_trainer("job-lora-1")
      self.manager.shutdown("job-lora-1")
      self.manager.shutdown("job-lora-1")

    self.assertEqual(self.api.deleted, ["lora-qwen-qwen2-5-0-5b-0-trainer"])
