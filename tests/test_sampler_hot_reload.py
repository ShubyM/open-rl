...
class EngineKwargsTest(unittest.TestCase):
  def test_lora_mode_keeps_lora_engine_kwargs(self) -> None:
    with patch.dict(os.environ, {}, clear=True):
    ...
    self.assertEqual(kwargs["max_loras"], 8)
    self.assertEqual(kwargs["max_lora_rank"], 64)
    self.assertNotIn("hf_overrides", kwargs)

  def test_language_model_only_env_override_in_lora_mode(self) -> None:
    with patch.dict(os.environ, {"OPEN_RL_VLLM_LANGUAGE_MODEL_ONLY": "0"}, clear=True):
      kwargs = vllm_sampler.build_engine_kwargs("base-model")

    self.assertFalse(kwargs["language_model_only"])

  def test_fft_mode_enables_sleep_and_disables_lora(self) -> None:
    ...
    self.assertNotIn("max_lora_rank", kwargs)

  def test_language_model_only_env_override_under_fft(self) -> None:
    env = {"OPEN_RL_ENABLE_FFT": "true", "OPEN_RL_VLLM_LANGUAGE_MODEL_ONLY": "0"}
...
class ArchitectureOverrideWarningTest(unittest.TestCase):
  ...
  def test_language_model_only_disabled_never_warns(self) -> None:
    env = {"OPEN_RL_ENABLE_FFT": "true", "OPEN_RL_VLLM_LANGUAGE_MODEL_ONLY": "0"}
    with patch.dict(os.environ, env, clear=True):
      self.assertFalse(vllm_sampler.architecture_override_missing_for_fft())
