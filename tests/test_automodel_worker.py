...
class AutomodelWorkerHelpersTest(unittest.TestCase):
  ...
  def test_stage_and_swap_uses_rank_invariant_staging_path(self):
    ...
    from training.automodel_worker import AutomodelTrainer
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
      ...
      seen: list[str] = []
      trainer.write_weights = lambda path: seen.append(path) or (open(f"{path}/w", "w").close() if os.path.isdir(path) else None)  # type: ignore[method-assign]
      with patch("training.automodel_worker.is_primary", side_effect=[True, True, False, False]):
        trainer.save_state(target)
        trainer.save_state(target)
