...
def get_default_model_name() -> str | None:
  if is_single_process_mode():
    ...
    if worker_mod.engine.base_model_name:
      return worker_mod.engine.base_model_name
...
async def lifespan(app: FastAPI):
  ...
  if is_single_process_mode():
    ...
    if base_model:
      await asyncio.to_thread(worker_mod.engine.load_base_model, base_model)
    task = asyncio.create_task(worker_mod.clock_cycle_loop())
  try:
    yield
  finally:
    if task is not None:
...
async def save_weights_for_sampler(req: dict):
  """TrainingClient.save_weights_for_sampler() — instant resolve, worker auto-saves after optim_step."""
