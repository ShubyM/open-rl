...
class TrainerEngine:
  ...
  def optim_step(self, model_id: str, learning_rate: float, grad_clip_norm: float | None = None) -> dict[str, Any]:
    ...
    if model_id not in self.optimizers:
      ...
      params = [p for p in self.peft_model.parameters() if p.requires_grad]
      self.optimizers[model_id] = torch.optim.AdamW(params, lr=learning_rate)
