...
```bash
python3 collect.py --task banking-finance/identify-issues-in-commitment-letter --max-attempts 2 --parallel 1
```

Split run:

```bash
python3 collect.py --split split.json --subset train --model gemini-3.5-flash --judge-model gemini-3.5-flash --parallel 4
```
