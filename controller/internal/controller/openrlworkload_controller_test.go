...
func worker(name, modelID string, role openrlv1alpha1.WorkerRole, memory string) *openrlv1alpha1.OpenRLWorkload {
	return &openrlv1alpha1.OpenRLWorkload{
		ObjectMeta: metav1.ObjectMeta{
			Name:              name,
			Namespace:         testNamespace,
			CreationTimestamp: metav1.Now(),
		},
		Spec: openrlv1alpha1.OpenRLWorkloadSpec{
			Role:        role,
			ModelID:     modelID,
...
func trainerWorker(name, modelID string) *openrlv1alpha1.OpenRLWorkload {
...
func ownedWorker(name, modelID, owner string) *openrlv1alpha1.OpenRLWorkload {
...
func fillerWorker(name, modelID string) *openrlv1alpha1.OpenRLWorkload {
	return worker(name, modelID, openrlv1alpha1.RoleTrainer, "90Gi")
}

func newReconciler(t *testing.T, objects ...client.Object) *OpenRLWorkloadReconciler {
	t.Helper()
	c := fake.NewClientBuilder().
		WithScheme(testScheme(t)).
		WithObjects(objects...).
		WithStatusSubresource(&openrlv1alpha1.OpenRLWorkload{}, &corev1.Pod{}).
		Build()
	return &OpenRLWorkloadReconciler{
		Client:           c,
		Namespace:        testNamespace,
		...
		// Long enough that only tests that backdate a claim trigger the
		// fall-back-to-sharing move.
		ScaleOutGracePeriod: time.Hour,
	}
}

func runReconcile(t *testing.T, r *OpenRLWorkloadReconciler, name string) ctrl.Result {
	t.Helper()
...
func settle(t *testing.T, r *OpenRLWorkloadReconciler, names ...string) {
	t.Helper()
...
// shareAfterGrace ages the worker's dedicated claim past the scale-out
// grace, plays kube-scheduler declining the pod, and reconciles twice: once
// to move the seat and drop the stale pod, once to rebuild it on the shared
// claim. If no claim is joinable the worker stays dedicated.
func shareAfterGrace(t *testing.T, r *OpenRLWorkloadReconciler, name string) {
	t.Helper()
	var claim resourcev1.ResourceClaim
	claimName := claimOf(t, r, name)
	if err := r.Get(context.Background(), types.NamespacedName{Namespace: testNamespace, Name: claimName}, &claim); err != nil {
		t.Fatalf("get claim %s: %v", claimName, err)
	}
...
func markUnschedulable(t *testing.T, r *OpenRLWorkloadReconciler, name string, since time.Time) {
	t.Helper()
...
func getWorker(t *testing.T, r *OpenRLWorkloadReconciler, name string) *openrlv1alpha1.OpenRLWorkload {
	t.Helper()
	var w openrlv1alpha1.OpenRLWorkload
...
func claimOf(t *testing.T, r *OpenRLWorkloadReconciler, name string) string {
	t.Helper()
...
func getPod(t *testing.T, r *OpenRLWorkloadReconciler, name string) *corev1.Pod {
	t.Helper()
	var pod corev1.Pod
...
func allocateClaim(t *testing.T, r *OpenRLWorkloadReconciler, name string) {
	t.Helper()
	var claim resourcev1.ResourceClaim
...
func TestReconcilePlacesUnplacedWorker(t *testing.T) {
	...
	// API server writes it, the controller stamps only the group.
...
func TestReconcileRejectsPlacementShapedTemplates(t *testing.T) {
	...
	r := newReconciler(t, append(enabledNode(1), w)...)
...
func TestReconcileDerivesTheDeviceCountFromMemory(t *testing.T) {
	...
	r2 := newReconciler(t, append(enabledNode(4), undeclared)...)
...
func TestReconcileRecreatesPodBoundToAStaleClaim(t *testing.T) {
	...
	w.Status = openrlv1alpha1.OpenRLWorkloadStatus{
	...
	old := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			OwnerReferences: []metav1.OwnerReference{{
				APIVersion: "openrl.io/v1alpha1", Kind: "OpenRLWorkload",
...
func TestReconcileSpreadsThenSharesUnderContention(t *testing.T) {
	...
	// The grace elapses with c still unallocated: c falls back to sharing.
	shareAfterGrace(t, r, "w-c")
	if got := claimOf(t, r, "w-c"); got != a && got != b {
		t.Errorf("w-c holds %q after the grace, want one of the allocated claims", got)
	}

	// The abandoned claim is seatless and old; the sweep retires it.
	...
	if len(claims.Items) != 2 {
		t.Errorf("%d claims survive, want 2: one per device, the abandoned one swept", len(claims.Items))
	}
}
...
func TestDeletedWorkerHoldsItsSeatUntilThePodIsGone(t *testing.T) {
	...
	// w-c cuts its own dedicated claim, which DRA never satisfies. Even past
	// the grace it has nowhere to fall back to: the terminating worker still
	// holds claimA's one seat.
	runReconcile(t, r, "w-c")
	dedicated := claimOf(t, r, "w-c")
	shareAfterGrace(t, r, "w-c")
	if got := claimOf(t, r, "w-c"); got != dedicated {
		t.Fatalf("w-c moved to %q, but the terminating worker still holds its seat", got)
	}
