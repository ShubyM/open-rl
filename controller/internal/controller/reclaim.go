...
// Claims carry no owner reference -- a shared claim belongs to no single
// worker, so GC'ing it with its creator would pull the allocation out from
// under everyone else -- which is why this sweep must exist. Four stays of
// execution: an OpenRLWorker names it, a live pod sits on it, DRA still
// reserves it, or it is too young to judge.
