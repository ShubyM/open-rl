...
// readFleet folds ResourceSlices, node labels, claims and group seats into
...
func (r *OpenRLWorkloadReconciler) readFleet(ctx context.Context) (*placement.Fleet, error) {
	...
	if err := r.List(ctx, &groups, client.InNamespace(r.Namespace), client.MatchingLabels{LabelManaged: "true"}); err != nil {
		return nil, fmt.Errorf("list claim groups: %w", err)
	}
	for i := range groups.Items {
		group := &groups.Items[i]
		claim, ok := fleet.Claims[group.Spec.ClaimName]
		if !ok {
			continue
		}
...
func (r *OpenRLWorkloadReconciler) poolsFrom(ctx context.Context, slices []resourcev1.ResourceSlice, nodes []corev1.Node) map[string]*placement.Node {
...
// claimFrom reads a managed claim back into the fleet: identity and age from
...
// requestFrom is the placement Request an OpenRLWorkload spec is asking for.
// Validation is the CRD schema's job.
func requestFrom(worker *openrlv1alpha1.OpenRLWorkload) placement.Request {
	spec := worker.Spec
...
func hostRequestBytes(worker *openrlv1alpha1.OpenRLWorkload) int64 {
