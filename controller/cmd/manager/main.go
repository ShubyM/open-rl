...
//
// It watches OpenRLWorkload resources and reconciles each into a DRA
...
func main() {
	...
	flag.DurationVar(&scaleOutGrace, "scale-out-grace-period", envDuration("OPEN_RL_SCALE_OUT_GRACE_PERIOD", 15*time.Second),
		"How long a dedicated claim may wait for DRA or the autoscaler before its worker falls back to sharing. "+
			"Raise to node-provisioning time (3-5m) on fleets with GPU autoscaling; 0 never falls back.")
	flag.IntVar(&maxConcurrent, "max-concurrent-reconciles", envInt("OPEN_RL_MAX_CONCURRENT_RECONCILES", 4),
		"How many workers place at once. Seat booking is CAS-arbitrated, so concurrency risks only transient over-cut claims, which the scale-out fallback drains.")
	...
	reconciler := &controller.OpenRLWorkloadReconciler{
		...
		ScaleOutGracePeriod:     scaleOutGrace,
		MaxConcurrentReconciles: maxConcurrent,
	}
	if err := reconciler.SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "cannot set up the OpenRLWorkload controller")
		os.Exit(1)
	}
