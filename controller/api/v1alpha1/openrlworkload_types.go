...
// Condition types set on an OpenRLWorkload.
const (
...
// OpenRLWorkloadSpec is one worker process's scheduling request. Memory arrives
...
type OpenRLWorkloadSpec struct {
...
// OpenRLWorkloadStatus is what the controller decided and what came of it.
type OpenRLWorkloadStatus struct {
	...
	// itself lives on the claim's OpenRLClaimGroup, whose name is derived
	// from the claim's.
	ClaimName string `json:"claimName,omitempty"`

	// AssignmentID is this worker's current booking in that group. It must
	// match the group's seat and the pod's stamp; a mismatch marks a stale
...
// OpenRLWorkload is the scheduling request for a single worker process. The
...
// status -- so `kubectl get openrlworkloads` shows what was asked for, where it
// went, and why a pending worker is still pending.
type OpenRLWorkload struct {
	...
	Spec   OpenRLWorkloadSpec   `json:"spec"`
	Status OpenRLWorkloadStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// OpenRLWorkloadList is a list of OpenRLWorkload.
type OpenRLWorkloadList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []OpenRLWorkload `json:"items"`
}

func init() {
	SchemeBuilder.Register(&OpenRLWorkload{}, &OpenRLWorkloadList{})
}
