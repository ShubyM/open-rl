...
def plot_metrics(file_path: str, output_file: str | None = None) -> bool:
    ...
    metric_columns = [
        ...
        if column != "step" and pd.api.types.is_numeric_dtype(df[column])
    ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Plot numeric metrics from a JSONL file against `step` (or row order if `step` is missing)."
    )
    parser.add_argument(
        "file_path",
        nargs="?",
