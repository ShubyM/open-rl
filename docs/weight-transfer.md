...
workload. If the time slicer cannot park the process, it exits without
unregistering so the accelerator cannot be reassigned while still in use.
...
`strategy`: `delta` (default) writes sparse patches on explicit export, `full` writes a
...
Sparse transfer sends indices and changed values, but does not eliminate
dense GPU staging. For dense updates, indices can make the payload larger
than a full checkpoint. Only loaders supported by the upstream sparse-copy contract
