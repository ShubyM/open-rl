...
On Nodes, the inspector stays below the fleet row where it was opened. **This node** identifies the accelerator and node, with GPU selectors above its active run rows. **Across nodes** identifies the selected run; choosing a process updates the GPU section below it without moving the inspector. The selected row and blocks identify the process, and each node remembers its GPU selection while switching. Return to **This node** to see the other active runs on those GPUs.
...
- `#nodes?placement={id}&gpu={device_id}&group={anchor_placement_id}&layout=across` opens a run across nodes. Omit `layout` for active runs on this node, `gpu` for all GPUs, and `group` when the selection also anchors the GPU group.
