...
- **Nodes**: one lane per node, one row per GPU, allocation bars over the selected window from placement history. Click a bar for a full-width activity timeline. Direct process buttons, such as **Trainer · L4 · ozuv**, select the node and GPU group. The default timeline shows individual run rows for the selected GPUs; runs with no recorded activity in the window are omitted. **Compare run** toggles a comparison of the selected run's processes on a shared time scale without changing the selected node. Only one activity timeline is shown at a time, with a smaller GPU utilization chart below. Switching scopes keeps the selected run, GPU group and time window. Select a block or run label without changing the GPU group; GPU buttons select individual cards. Pinch or Ctrl/⌘-scroll over a timeline to zoom around the pointer; drag empty timeline space to pan. Ordinary scrolling still moves the page. The picker, activity timeline and utilization chart share the same window, from one minute to 24 hours within the snapshot’s retained day. GPU queries wait until gestures finish; cached samples are redrawn while moving. Loading and telemetry errors remain visible even when empty run rows are omitted. Blocks provide operation context, not per-job utilization measurements or proof that a gap was idle.
...
On Nodes, the inspector stays below the fleet row where it was opened. Its heading always identifies the selected run. Process buttons name each role, accelerator and node; separate buttons are retained when trainer and sampler share a node. The process buttons and GPU selectors keep the same position in both timeline views. Choosing a process changes the selected hardware without changing the comparison toggle, and each node remembers its valid GPU selection. The utilization graph names its node; in comparison mode, GPU selectors affect that graph while the timeline shows all assigned GPUs. Turn off **Compare run** to see the other active runs on the selected GPUs.
...
- `#nodes?placement={id}&gpu={device_id}&group={anchor_placement_id}&layout=across` opens a run comparison. Omit `layout` for active runs on the selected node, `gpu` for all GPUs, and `group` when the selection also anchors the GPU group.
