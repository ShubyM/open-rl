...
Under `OPEN_RL_ENABLE_FFT=true` the sampler starts with `enable_sleep_mode=True`
and `enable_lora=False`; LoRA adapter options apply only to the default LoRA
mode. The vLLM pin is `vllm==0.25.1.dev29+gf378f79b7` (wheels.vllm.ai): the
0.25.0 tag has the hot-reload surface (`reload_weights` worker RPC, tagged
`wake_up`, `language_model_only`), but the dev build adds the checkpoint
integrity check that makes a corrupted or half-written checkpoint directory
fail loudly at reload instead of silently loading a mix of old and new
shards.
