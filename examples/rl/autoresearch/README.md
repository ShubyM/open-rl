...
| --- | --- | --- | --- | --- |
| Text-SQL | `rl.autoresearch.text_sql.train` | `train.py` | `accuracy` | [Text-SQL](text_sql/README.md) |
| Math-RL | `rl.autoresearch.math_rl.train` | `config.toml` | `accuracy` | [Math-RL](math_rl/README.md) |
...
kubectl apply -k examples/rl/autoresearch/math_rl

# Convenience one-shot Math-RL stack: Open-RL backend + autoresearch add-on.
kubectl apply -k examples/rl/autoresearch/math_rl/gke
```
...
GPU, storage, and the Open-RL backend. These overlays add researcher jobs and
the UI on top of that shared backend.

Researcher pods wait for comma-separated `READY_URLS` before the baseline, so
...
attempts inside this window: edit, commit, run attempt, decide keep/reset, then
repeat. Setup and baseline happen before this clock starts.
