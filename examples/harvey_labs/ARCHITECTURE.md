...
runs being compared. So does `train_tasks`: eval is the slice
`shuffled[train_tasks : train_tasks + eval_tasks]`, so changing the train count
silently moves the benchmark.

- `task_set=random` (default) — two-way train/eval split, disjoint by task.
- `task_set=disjoint` — three-way sft/train/eval split, disjoint by task
  *family*, eval stratified across practice areas.

`train_split_seed` (random only) decouples the two: `task_split_seed` still
fixes eval, and the train pool is redrawn from every task outside eval's
scenario families. That family filter matters — the plain seed-0 draw is
disjoint from eval by task name but not by family, so 8 of its 300 train tasks
are scenario siblings of eval tasks (a different scenario of the same MSA
first-draft, say), which leaks benchmark structure into training.

Candidate train seeds were ranked over the 1,674-task filtered pool on
distinct scenario families, practice-area L1 against the pool distribution, and
count of tasks whose document set exceeds the pool p95 (context pressure).
Median criteria count (~57) and median document bytes (~338 KB) are flat across
seeds, so they don't discriminate. **`train_split_seed=242`** wins: 293 distinct
families (seed-0 draw: 282, best available: 295), area-L1 0.125 (seed 0: 0.187),
16 oversized-document tasks (seed 0: 19), 0 leaked siblings.

A new draw cannot be scored empirically — only 271 of the 1,749 tasks have ≥3
graded episodes, all inside the seed-0 split, so a fresh 300 overlaps ~26–49
tasks of known difficulty. Rank on structure, then measure.
