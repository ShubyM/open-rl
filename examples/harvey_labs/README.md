...
uv --project examples run python -m harvey_labs.train \
...
default, preflighted for instructions/criteria/documents. Eval is a fixed
slice of the seeded shuffle (`EVAL_SLICE_OFFSET` in `tasks.py`), so it
depends only on `task_split_seed` and `eval_tasks`. Keep those two fixed
across runs you want to compare and the benchmark is byte-identical; the
train count can change freely. Train is drawn from every task outside
eval's scenario families, so no scenario sibling of an eval task leaks
into training.
...
- **`plot_run.py`** — rewards and held-out pass rate from a run directory:
  `uv --project examples run python -m harvey_labs.plot_run artifacts/harvey-labs/<run>`
